...
def configure_logging(dry_run: bool = False, log_dir_path: str = "logs") -> None:
    """ログ設定を行う。dry-run時はStreamHandlerのみ、本運用時はFileHandlerも追加。"""
...
def _count_symbols_from_json(watchlist_file: str) -> int:
    """JSONファイルから銘柄数をカウントする（DBを使わずread-only）。"""
    import json
...
def run_cycle(target_date: str, dry_run: bool = False, config_path: str = "config.yaml") -> dict:
    ...
    if not dry_run:
        ...
        # dry-run: DBを使わずJSONからread-onlyで検証
        symbols_count = _count_symbols_from_json(config.watchlist_file)
        results["symbols"] = symbols_count
        codes = []
        symbols_info = {}
        benchmark_codes = set()
