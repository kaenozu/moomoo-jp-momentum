"""Temporary branch-local patch applicator; deleted by its own commit."""
...
TEST_CONTENT = '''"""Regression coverage for as-of-date virtual-order validation."""
...
def main() -> None:
    ...
    for old, new in SOURCE_REPLACEMENTS:
        ...
        if count != 1:
            raise RuntimeError(
                f"expected one source match, found {count}: {old[:80]!r}"
            )
        source = source.replace(old, new, 1)
    source_path.write_text(source, encoding="utf-8")

    Path("tests/test_virtual_trade_order_dates.py").write_text(
        TEST_CONTENT,
        encoding="utf-8",
    )

    pyright_path = Path("pyrightconfig.json")
    pyright = pyright_path.read_text(encoding="utf-8")
    marker = '    "tests/test_virtual_trade_reservations.py"\n'
    replacement = (
        '    "tests/test_virtual_trade_order_dates.py",\n'
        '    "tests/test_virtual_trade_reservations.py"\n'
    )
    if marker not in pyright:
        raise RuntimeError("pyright insertion marker not found")
    pyright_path.write_text(
        pyright.replace(marker, replacement, 1),
        encoding="utf-8",
    )
