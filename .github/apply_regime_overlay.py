...
def update_regime_config(path: str) -> None:
    file_path = Path(path)
    lines = file_path.read_text(encoding="utf-8").splitlines()
    try:
        start = lines.index("  idle_cash_allocation:")
    except ValueError as exc:
        raise RuntimeError(f"idle_cash_allocation not found: {path}") from exc
    ...
    benchmark_index = next(
        (
            ...
            if line.strip().startswith("benchmark_code:")
        ),
        None,
    )
    if benchmark_index is None:
        raise RuntimeError(f"benchmark_code not found: {path}")

    cleaned: list[str] = []
    skipping_regime = False
    for line in block:
        indent = len(line) - len(line.lstrip())
        if line.strip() == "regime:" and indent == 4:
            skipping_regime = True
            continue
        if skipping_regime:
    ...
    cleaned[benchmark_index + 1 : benchmark_index + 1] = regime_lines
    lines[start:end] = cleaned
    file_path.write_text("\n".join(lines) + "\n", encoding="utf-8")


update_regime_config("config.yaml")
update_regime_config("config.example.yaml")
...
class RegimeDetector:
    ...
    def detect(self, df: pd.DataFrame) -> list[dict]:
        ...
        for _, row in data.iterrows():
            ...
            if pd.isna(close) or pd.isna(sma_short) or pd.isna(sma_long):
                results.append(
                    {
                        "date": date,
                        "regime": "unknown",
...
for path in (
    ".github/apply_regime_overlay.py",
