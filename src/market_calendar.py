...
The calendar is checked in with the application so a scheduled run does not
need a network request to decide whether the JPX cash market is open. A date
outside the checked-in range is an operational configuration error, not a
reason to silently skip work.
"""
...
class JPXMarketDayStatus:
    """Auditable result of one JPX market-day check."""

    target_date: date
    is_trading_day: bool

    def as_result(self) -> dict[str, bool | str]:
        return {
            "calendar_checked": True,
            "is_trading_day": self.is_trading_day,
            "cycle_skipped": not self.is_trading_day,
            "skip_reason": "jpx_market_closed" if not self.is_trading_day else "",
        }
...
def check_jpx_market_day(value: DateLike) -> JPXMarketDayStatus:
    target = _coerce_date(value)
    return JPXMarketDayStatus(target, get_jpx_calendar().is_trading_day(target))
