...
def calculate_metrics(
    ...
    *,
    ...
    returns = [
        current / previous - 1.0 for previous, current in zip(equity, equity[1:])
    ]
    ...
    sharpe = (
        mean / math.sqrt(variance) * math.sqrt(periods_per_year) if variance else 0.0
    )
    ...
    if benchmark_equity is not None:
        if len(benchmark_equity) != len(equity) or any(
            value <= 0 for value in benchmark_equity
        ):
            raise ValueError(
                "benchmark equity must align with positive strategy equity"
            )
        benchmark_cagr = (
            (benchmark_equity[-1] / benchmark_equity[0]) ** (1 / years) - 1
            if years
            else 0.0
        )
