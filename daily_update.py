    ...
    with sqlite3.connect(data_store.db_path) as conn:
        for _, row in indicators_df.iterrows():
            try:
                ...
                logger.error("指標保存エラー: %s - %s", row.get("code"), e)
...
def ensure_symbols_loaded(data_store: DataStore, symbols_file: str) -> int:
    """DBに有効銘柄がない場合、symbols.jsonから読み込む"""
    ...
    return len(data_store.get_enabled_symbols())
...
def main() -> int:
    ...
    parser.add_argument("--config", default="config.yaml", help="設定ファイルパス（デフォルト: config.yaml）")
    ...
    loaded_count = ensure_symbols_loaded(data_store, config.watchlist_file)
    symbols = data_store.get_enabled_symbols()
    ...
    if not codes:
        ...
        print(f"  {config.watchlist_file} を確認してください")
        return 1
    ...
    with OpenDConnection(config) as conn:
        ...
        display_cols = [
            ...
            "ma5", "ma25", "volume_ratio", "return_5d", "history_days",
        ]
