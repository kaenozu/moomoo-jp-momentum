...
def validate_human_payload(payload: Any) -> tuple[list[str], dict[str, int]]:
    errors: list[str] = []
    ...
    if not isinstance(reviewed_at, str) or not parse_datetime(reviewed_at):
        errors.append("reviewed_at must be an RFC3339 timestamp with timezone")
    ...
    for name in REQUIRED_CHECKS:
        ...
        if not isinstance(item, dict):
            ...
            continue
        extra = sorted(set(item) - {"status", "value", "evidence_refs", "notes"})
        if extra:
            errors.append(
                f"checks.{name} contains unknown fields: {', '.join(extra)}"
            )
        ...
        if not isinstance(refs, list) or any(
            ...
            errors.append(
                f"checks.{name}.evidence_refs must contain non-empty strings"
            )
            refs = []
        elif len(refs) != len(set(refs)):
            errors.append(f"checks.{name}.evidence_refs contains duplicates")
        if status == "CONFIRMED":
            if not isinstance(value, str) or not value.strip():
                errors.append(f"checks.{name} is CONFIRMED but value is empty")
            if not refs:
                errors.append(
                    f"checks.{name} is CONFIRMED but evidence_refs is empty"
                )

    if payload.get("authorization_acknowledgement") != EXPECTED_ACKNOWLEDGEMENT:
        errors.append(
...
def validate_operator_result(payload: Any) -> list[str]:
    ...
    if not authorization_is_fail_closed(payload):
        errors.append("operator result authorization boundary is not fail-closed")
    return errors
...
def validate_discovery(payload: Any) -> list[str]:
    ...
    if not isinstance(gate, dict):
        ...
        if gate.get("gate_passed") is not True:
            errors.append("redacted discovery gate_passed must be true")
        if gate.get("discovery_executed") is not True:
            errors.append("redacted discovery discovery_executed must be true")
    ...
    if not authorization_is_fail_closed(discovery.get("authorization")):
        errors.append("redacted discovery authorization is not fail-closed")
    if not authorization_is_fail_closed(payload.get("authorization")):
        errors.append("gated result authorization is not fail-closed")
    return errors


def validate_release_manifest(payload: Any) -> tuple[list[str], bool]:
    if not isinstance(payload, dict):
    ...
    if payload.get("operator_version") != "1.2.1":
        errors.append("release manifest operator_version must be 1.2.1")
    commit = payload.get("source_commit")
    if not isinstance(commit, str) or HEX40.fullmatch(commit) is None:
        errors.append("release manifest source_commit must be 40 lowercase hex")
    if not authorization_is_fail_closed(payload.get("authorization")):
        errors.append("release manifest authorization is not fail-closed")
    candidate = payload.get("release_candidate") is True
    if candidate:
        if payload.get("source_ref") != "refs/heads/master":
            errors.append(
                "release_candidate=true requires source_ref refs/heads/master"
            )
        if payload.get("distribution_status") != "MASTER_RELEASE_CANDIDATE":
            errors.append(
                "release_candidate=true requires MASTER_RELEASE_CANDIDATE"
            )
        if payload.get("source_event") != "push":
            errors.append("release_candidate=true requires source_event push")
...
def evaluate(
    ...
    result = {
        ...
        "machine_validation_status": operator_result.get(
        ...
        "next_action": {
            ...
            "CORRECTION_REQUIRED": (
                "Correct malformed or contradictory evidence before retrying."
            ),
        }[status],
        "authorization": {
            **EXPECTED_AUTHORIZATION,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    paths = {
        "human_validation": Path(args.human_validation).resolve(),
        "operator_result": Path(args.operator_result).resolve(),
        "discovery_redacted": Path(args.discovery_redacted).resolve(),
