...
使い方:
    # JP市場では実行不可（エラーになる）
    python paper_order.py --code JP.7203 --side BUY --quantity 1 --price 2700

    # US市場のみ（明示的にexperimental指定が必要）
    python paper_order.py --market US --experimental --code US.AAPL --side BUY --quantity 1 --price 200
"""
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Moomoo ペーパートレードCLI（experimental）"
    )
    parser.add_argument(
        "--code",
        help="銘柄コード（例: JP.7203 or US.AAPL）",
    )
    parser.add_argument(
        "--side",
        choices=["BUY", "SELL"],
    ...
    # JP市場は実行不可
    if args.market == "JP" and not args.experimental:
        print_unsupported()
        return 1

    # US市場はexperimental指定が必要
    ...
    # US市場のexperimental機能
    if args.market == "US" and args.experimental:
        ...
        if args.cancel:
            if not args.order_id:
                ...
                return 1
            cancelled = manager.cancel_order(args.order_id)
            if cancelled:
